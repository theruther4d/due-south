...
var Main = ( function() {
    return {
        init: function() {
            ...
            const items = Array.from( document.querySelectorAll( '.testVideoCanvas' ) );
            items.forEach( ( item ) => {
                scrollMonitor.addItem( item, 0, 10000, ( progress ) => {
                    // item.style.backgroundColor = `rgba( 255, 0, 0, ${progress / 1000} )`;
                    // console.log( progress );
                    item.style.transform = `scale( ${progress / 10000} ) translateZ( 0 )`;
                });
            });
